...
//		public String rango, tipo, nombre, valor, renglon, columna;
			...
			if (tipo[i] == entero || tipo[i] == booleano) {
				...
				if (tipo[i - 1] == publico) {
					if (nombreTipo.equals("int"))
						ValoresHaciaTabla("public", nombreTipo, nombre[i + 1], "0", renglon[i + 1]);
					else
						ValoresHaciaTabla("public", nombreTipo, nombre[i + 1], "false", renglon[i + 1]);

				} else if (tipo[i - 1] == privado) {
					if (nombreTipo.equals("int"))
						ValoresHaciaTabla("private", nombreTipo, nombre[i + 1], "0", renglon[i + 1]);
					else
						ValoresHaciaTabla("private", nombreTipo, nombre[i + 1], "false", renglon[i + 1]);
				} else {
					if (nombreTipo.equals("int"))
						ValoresHaciaTabla("S/M", nombreTipo, nombre[i + 1], "0", renglon[i + 1]);
					else
						ValoresHaciaTabla("S/M", nombreTipo, nombre[i + 1], "false", renglon[i + 1]);
				}
	...
	public void ValoresHaciaTabla(String ran, String tip, String nom, String val, String reng) {
		valoresTab.add(new ValoresTabla(ran, tip, nom, val, reng));
	}
